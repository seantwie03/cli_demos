...
Reviewed the working tree on 2026-09-12, including the Python entry point, package, tools, tests, README, and changes to the original Bash script. Updated on 2026-09-13 after implementing Finding 6’s recording lifecycle and KDE/Niri placement, following the snapshot, navigation, explicit-key, and socket updates. Original diagnoses and design proposals are retained as history; older numeric source references may have shifted with the implementation.

**Current status:** Finding 1 is fixed within the accepted snapshot/timing limitations. Finding 2 is fixed. Finding 4 now has the agreed explicit navigation/recovery workflow and full-height HUD; manual terminal state remains the presenter's responsibility. Finding 5 is partially addressed. Findings 3, 7, 10, and 12 remain open. Finding 6 is implemented, including KDE/Niri placement; Finding 9 is partly addressed through owned window IDs. Finding 8 is resolved, and Finding 11 now has the documented socket setup and explicit startup errors. The smaller display-content issue is resolved, and several documentation/completion messages were corrected with the HUD update.
...
### 6. Resolved (originally P2) — Recording ownership and verified finalization

**Status:** Implemented on 2026-09-13. `Session` owns the returned Kitty window ID from launch through rollback/finalization. Record mode runs a supervisor as the Presentation process; it waits for asciinema before issuing a completion receipt. The CLI publishes only after successful finalization and window closure. KDE placement is added and Niri support retained.

**Current implementation:** `kitty-demo.py` (`run()` and controller cleanup), `kittydemo/driver.py` (`Session`, `launch()`, `stop_recording()`), `kittydemo/recorder.py` (supervision and completion receipt), and `kittydemo/placement.py` / `placement.js` (desktop placement).

#### Implemented lifecycle

- All presentation input, outer-PTY lookup, and closure use the owned Kitty ID. A unique per-run user variable permits recovery if a launch response is lost; a unique application ID identifies the compositor window. The existing controller PID-file and leftover-window policy remain, so Finding 9 is only partly addressed.
- Startup now rolls back on exceptions and interruption. Desktop placement prepares before launch and warns on ordinary placement failure. The supervisor starts before the recorder, preserving the accepted outer/inner PTY snapshot boundary. Empty PTY differences get a bounded retry; ambiguous differences still fail. Prompt customization occurs only after PTY selection. Bash initialization still has an explicit timing assumption.
- The recorded shell reports its process identity to the supervisor, which opens a Linux pidfd before allowing it to exec interactive login Bash. On stop, the supervisor signals that shell with SIGHUP and waits for its asciinema child to exit. A zero recorder exit after a stop request, without forced termination or supervisor error, is required. This is a process-status handshake, not a replacement for snapshot TTY discovery. It does not infer demo-command success or readiness.
- A ten-second graceful-stop deadline is followed by terminate/kill fallback when necessary. Forced termination cannot publish successfully. The controller checks for early recorder exit before playback actions and before finalization. After a successful receipt it closes only the owned window and verifies disappearance; then the CLI flushes the partial file with `fsync` and atomically replaces the destination.
- Every attempt gets a distinct, privately created partial file beside the destination. Failure preserves it and prints its pathname; retries cannot overwrite earlier partials. Recorder control files are normally removed after cleanup, but are retained and reported if recorder cleanup cannot be verified. Secondary cleanup errors are warned about without replacing the original failure. The unsupported `pnpm run casts` completion instruction was removed.
- Live presentations remain open after successful startup, including on later interruption. Startup failures close an owned, partially initialized live window. Controller title and PID-claim cleanup both run even if one cleanup step fails.

**Verification:** All 115 automated tests pass; `git diff --check`, Python compilation, and shell syntax checks also pass. Automated coverage includes publication ordering, preservation of old casts and partials, distinct retry paths, startup rollback, early recorder exit, unsuccessful completion receipts, forced termination, close failure, deadlines, recovery by unique marker, ID-only closure, and interruption cleanup. A committed test runs the installed asciinema under an isolated real PTY, selects the inner device by snapshot, captures a header marker, and checks its waited-for completion receipt. An isolated graphical Kitty instance on KDE Wayland also captured a header, finalized, published, and closed only its Presentation. The KDE acknowledgement verified its output and full maximization on the laptop's single monitor. Physical multi-monitor movement and a live Niri rehearsal remain unverified.

#### Historical diagnosis before this implementation

The following analysis records the prior behavior and the design rationale; the gaps listed here were addressed by the implementation above.

#### What has changed

- Finding 11 now rejects an unreachable Kitty instance or invalid window listing during `claim_session()`, before writing the PID claim or launching a Presentation window. `main()` reports `KittyConnectionError` without a traceback and returns 1. This fixes that pre-launch failure path; it does not finalize a recording or clean up a window created before a later connection failure.
- Finding 1 now validates the outer recording PTY and requires exactly one new presentation PTY. These checks improve discovery, but can raise after window creation. `launch()` still has no internal rollback.
- Ctrl-C and EOF now terminate with explicit exit statuses, and the normal outer cleanup restores the Controller title and releases its PID claim. The recording HUD says it is finalizing rather than claiming the recorder has already stopped. These improvements do not verify recorder completion.

#### Original lifecycle gaps

1. **Window ownership begins before cleanup protection.** `run()` still calls `driver.launch()` before entering its `try/finally`. If launch creates a window and then fails in Niri handling, a remote-control request, prompt setup, or PTY discovery, recording teardown is skipped. The outer `_main()` cleanup normally releases the controller claim, but leaves that Presentation window behind; the next attempt is then rejected by the title check. This startup leak can affect live mode too. The missing recording-output argument is checked before window creation and is not part of this leak. The returned Kitty window ID is still discarded, so cleanup cannot yet target an owned window by ID.

2. **Playback completion is treated as recording completion.** `completed = True` means only that `play()` returned. If recording teardown raises an ordinary exception, the code prints a warning and still replaces the destination with the partial file. Provided replacement succeeds, it prints “Recording written” and returns 0. The previous good cast can therefore be replaced even when shutdown is known to have failed.

3. **Close failures and recorder completion are not checked.** `stop_recording()` sends a title-matched `close-window` with `check=False` and discards the returned status and stderr. A nonzero response therefore need not even produce the warning above. There is no wait for recorder exit, exit-status check, or confirmation that output has finished flushing. Switching these requests to a Unix socket changes transport, not this lifecycle. The docstring's assertion that closing yields a complete, well-formed cast is stronger than the code verifies; successful close-request delivery alone is insufficient evidence.

4. **Failed partial files receive inconsistent treatment.** If playback raises, `run()` attempts teardown and unlinks the partial even if teardown failed and the recorder may still be writing. That preserves the old destination but removes the partial pathname needed for diagnosis or recovery. A launch failure instead bypasses this block and can leave a partial behind. Every run uses the same `.cast.partial` name, and the recorder is started with `--overwrite`, so a retry can overwrite that remaining evidence. An unlink or replacement error can also obscure an earlier failure or produce an unhandled filesystem exception.

#### Pre-implementation verification and recommendation

Fresh isolated probes used temporary cast files and mocked presentation effects; they did not touch a running Kitty window or recorder. Both a teardown exception and a nonzero close response replaced a marker representing the previous good cast with an unfinished marker and returned 0. A playback exception preserved the old cast but removed the partial. A launch exception skipped teardown and left the partial. These reconfirm the Python control flow, not actual recorder flushing or signal behavior. Existing automated tests cover pre-launch connection failure and live interruption cleanup, but do not yet establish successful recording finalization.

**Recommendation:** Give one session owner responsibility for cleanup from the moment a window is created, retaining its ID for precise targeting. Keep the accepted snapshot PTY design. Separate playback completion from verified recorder finalization, and publish with atomic replacement only after finalization succeeds. Report finalization failure as failure, preserve useful partial output with a reported pathname, and keep cleanup errors from hiding the original failure. Preserve the intentional live-mode behavior of leaving a successfully launched presentation open for questions. Window-ID targeting overlaps Finding 9; it does not require redesigning the singleton workflow before discussing the recording fix.

#### Implemented extension: retain Niri placement and add KDE

**Implementation status:** Added a small desktop backend boundary. KDE uses `gdbus` to load a unique temporary script and two unstarted marker scripts. The active script unloads each marker to acknowledge readiness or verified placement; the controller starts only that script object and unloads all owned names on exit. An independent script timer stops late placement even if the controller disappears. The application ID is passed through Kitty’s `--os-window-class`; the running KWin 6.7.5 accepted this mapping and confirmed output plus `maximizeMode == 3` during the isolated single-monitor recording test. Niri resolves the same unique ID, uses the pre-launch source output, and retains its helper with explicit focus before maximization. Niri’s final focus-dependent action can still race a concurrent focus change. The implementation contract below is retained as design context.

**Finding:** KDE can provide the requested behavior. This machine reports `XDG_CURRENT_DESKTOP=KDE`, `XDG_SESSION_TYPE=wayland`, and installed KWin 6.7.5. KWin exposes window/output enumeration, `workspace.sendClientToScreen(window, output)`, and `window.setMaximize(true, true)`. A short compositor script can therefore move the new Presentation to another monitor and maximize it. This is placement immediately after window creation, not a guarantee that its first frame appears on the destination. See the [KWin scripting API](https://develop.kde.org/docs/plasma/kwin/api/).

KDE also supports [Window Rules for screen placement](https://docs.kde.org/trunk_kf6/en/kwin/kcontrol/windowbehaviour/index.html), which are useful for a fixed display arrangement. For this project's automatic “other monitor” behavior, prefer a temporary script managed by the launcher: no user-installed KWin plugin, permanent window rule, or additional configuration step. Keep the existing Niri backend. Do not use X11-only window tools for the current Wayland session.

1. **Extract a small desktop-placement boundary.** Select Niri or KDE from the colon-separated `XDG_CURRENT_DESKTOP` tokens; unsupported desktops retain ordinary placement. Keep the Niri helper and add a KDE implementation alongside it. Use the available `gdbus` executable for KDE session-bus calls, preserving the Python standard-library-only design. Detect missing tools or an unavailable scripting interface and report a placement warning rather than requiring a new installation step.

2. **Define “other” before launch changes focus.** Capture the Controller's output before opening the Presentation; for an external recording invocation, use the active desktop output at that moment. With two outputs, choose the other one. With more, select the next output in a documented deterministic ordering (for example, connector name); with one, keep the new window there and maximize it. Revalidate the destination if outputs change during startup. Do not re-read the focused output after launching and accidentally choose the Controller's monitor as the destination. Apply the same policy to Niri where possible.

3. **Identify the owned window across both APIs.** Retain Kitty's returned window ID for terminal operations and cleanup. For KDE, give the new OS window a unique per-run application ID through Kitty's `--os-window-class` and match that identifier in KWin. The installed Kitty help confirms that this option sets the Wayland application ID. Keep the visible title `Presentation`. A Kitty window ID is not a KWin window identifier, and title-only matching can move an unrelated window. Verify the application-ID mapping on the installed KWin before relying on it. Niri can likewise resolve the unique application ID to its compositor window ID.

4. **Load a bounded, temporary KDE script.** Prepare it before launching so it can capture the source output and watch for the uniquely identified window. Use KWin's session-bus `/Scripting` interface to load it under a unique name, then start only that returned script object. Avoid the global `start()` method, which could start other loaded scripts. Use a readiness/result acknowledgement and a short deadline, not a fixed sleep as proof of placement. The script should move/maximize its target once, disconnect its handler, and report success or failure. The controller must unload only its own script and remove its temporary files on success, timeout, launch failure, or interruption. Define and verify the acknowledgement channel during implementation; a successful D-Bus load/start response alone does not prove that the target was found or placed. Use structured arguments and JSON serialization for generated script values.

5. **Integrate with Finding 6's ownership fix.** Establish cleanup ownership before either desktop helper or window creation. Treat placement failure as a warning and continue with the valid Presentation where it is; keep actual terminal/recorder startup failure fatal. Always release temporary placement resources, without losing the original error. Never let a delayed callback move a later run's window. Audit the retained Niri helper too: its move uses an explicit ID, but `maximize-column` acts on focus, and it currently maximizes even with one monitor despite its comment claiming no action. Preserve Niri support while making its target and single-monitor behavior explicit.

6. **Verify each backend independently.** Test desktop detection, deterministic output selection, unique-window matching, focus changes, missing tools, script load/run failure, timeout, hotplug, and unloading after interruption. Rehearse KDE with two monitors, an unrelated Kitty window, and a single-monitor setup; verify that only the new Presentation moves and maximizes. Retest Niri when that compositor is available. Confirm that a placement warning does not bypass recorder cleanup or alter publication decisions.

**Evidence and limits:** Initial read-only introspection confirmed KWin’s scripting interface. Implementation verification subsequently exercised script loading/execution, marker acknowledgement, application-ID matching, maximization, and unloading on the installed KWin. JavaScript tests with simulated outputs cover one/two/multiple monitors, focus changes, hotplug, expiry, and unrelated windows. Backend tests cover failures, interruption, cleanup, and Niri targeting. The graphical check used its own Kitty instance and temporary windows; no permanent KDE settings changed. Real physical multi-monitor movement and Niri compositor behavior remain rehearsal items.
...
### 9. Partly addressed (originally P2) — Singleton machinery is racy and imposes unnecessary window cleanup

**Current status:** Finding 6 now retains the launch ID and targets presentation effects/cleanup by ID, preventing title-based broadcasts from the Python driver. The PID-file race, leftover-Presentation restriction, and broad Controller mapping remain. The diagnosis below describes the original implementation.
...
4. **Implemented: recording lifecycle and desktop placement.** Keep the snapshot approach and configurable pauses. The supervisor provides verified recorder completion, startup rollback owns its window, and device validation precedes prompt setup. KDE and Niri placement use a small backend boundary. The new process-status handshake supports recording shutdown; TTY identity still uses snapshots.
5. **Move personal presentation policy out of the generic launch path.** The 55-pixel margins, forced 120×24 size, Bash-specific prompt mutations, and external cast-processing instructions are personal workflow choices. Keep the requested automatic monitor placement behind a small Niri/KDE backend boundary as proposed under Finding 6; move the other desktop/prompt policies into a small optional wrapper or documented local configuration. Do not replace them with a large framework of options. In particular, record mode chooses `$SHELL` but then sends Bash-oriented setup; choose a supported recording shell explicitly if that setup stays.
...
**Current verification after Finding 6:** All 115 tests pass, including 11 recording lifecycle/supervisor tests and seven desktop-placement tests added in this change. The real-PTY recorder test and isolated KDE graphical run verify recorded headers and successful finalization/publication; the graphical run also confirms single-monitor maximization. Physical multi-monitor KDE movement and live Niri behavior remain unverified. The older verification notes below describe earlier review stages; recording finalization is now covered as detailed under Finding 6.
